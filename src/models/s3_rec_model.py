...
from ._helper import Encoder, LayerNorm
...
class S3RecModel(nn.Module):
    def __init__(self, args):
        ...
        self.attribute_embeddings = nn.Embedding(
            args.attribute_size, args.model_args.hidden_size, padding_idx=0
        )
        ...
        self.aap_norm = nn.Linear(args.model_args.hidden_size, args.model_args.hidden_size)
        self.mip_norm = nn.Linear(args.model_args.hidden_size, args.model_args.hidden_size)
        self.map_norm = nn.Linear(args.model_args.hidden_size, args.model_args.hidden_size)
        self.sp_norm = nn.Linear(args.model_args.hidden_size, args.model_args.hidden_size)
    ...
    def associated_attribute_prediction(self, sequence_output, attribute_embedding):
        ...
        sequence_output = sequence_output.view(
            [-1, self.args.model_args.hidden_size, 1]
        )  # [B*L H 1]
    ...
    def masked_item_prediction(self, sequence_output, target_item):
        ...
        sequence_output = self.mip_norm(
            sequence_output.view([-1, self.args.model_args.hidden_size])
        )  # [B*L H]
        target_item = target_item.view([-1, self.args.model_args.hidden_size])  # [B*L H]
    ...
    def init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            ...
            module.weight.data.normal_(mean=0.0, std=self.args.model_args.S3RecModel.initializer_range)
